...
- 包含 v1.1.21 的官方 Harness `0.1.6-alpha.2` 升级、官方插件管理器迁移、联网搜索三模式、模型流空闲超时字段和缓存清理恢复。
- 修复 Linux 使用官方 LibreOffice Kit WASM 回退时被原生平台校验误拒的问题；现在明确校验 WASM loader、模块、数据和 metadata 四项声明及 SHA-256，同时只对真正的原生引擎要求目标平台与执行位。
