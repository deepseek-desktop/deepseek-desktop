...
| F06 | AppKit inset 读取派发到主线程并断言；调用前无持锁等待 | macOS 安装包菜单与设置交互通过；全屏仍复现 SIGABRT，不能归因线程修复已解决崩溃 |
...
| F04 | 候选从当前 Desktop 内置闭包取扩展，记录版本/摘要/入口/依赖并核对复制结果 | Node 闭包旧新版本替换、摘要变化、依赖缺失用例通过；实际候选切换待验 |
...
| SIGABRT | 保留崩溃报告，独立分析主线程及视图生命周期 | 新安装包 15:36:12 再现；LLDB 捕获主窗口 contentView 的 TaoView 仍被挂载却处于 deallocating，释放栈为自动释放池清理；具体过度释放来源未明，发布阻塞 |
...
新 DMG 已备份后安装至 /Applications；工作台可见、五组原生菜单展开、编辑快捷键、关闭确认取消、最小化唤回与小窗口设置滚动通过。设置文件及工作区数据与备份一致，但本机该 profile 没有可供验收的已有会话，不声称会话保持已真实验收。15:36:12 全屏过程中崩溃，故 macOS 整体验收失败；调试结束后恢复正常启动，不继续暂停用户实例。证据保存在忽略的 `target/audit/`。
...
Windows x64 原生安装验收、macOS 崩溃闭环、可信签名及最终四平台矩阵仍未通过。用户授权目标 1.1.0，但条件未满足；不得发布正式稳定版，保留现有 Release 和 Tag。
