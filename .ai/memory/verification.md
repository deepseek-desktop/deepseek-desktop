...
- `corepack pnpm@11.24.0 verify`：通过；配置与发行协议测试 80 项、Vue 测试 10 项、Rust 测试 65 项、Clippy `-D warnings`、3 个 locale / 140 个 key、凭据代理回归和 Runtime manifest 校验全部通过；另有 19 项跟随模型搜索测试覆盖自动协议映射、模型切换、多会话隔离、`CredentialRef` 继承、取消、超时、重定向和响应校验。
- `corepack pnpm@11.24.0 audit --prod --registry=https://registry.npmjs.org` 与 Runtime 子目录同项审计：均通过，无已知生产依赖漏洞。
- `corepack pnpm@11.24.0 test:e2e`：通过 2 项 Playwright 测试；除 Shell 自动启动、语言切换和状态视图外，还在 `1000x700` 的 Windows 尺寸视口验证长 Runtime 设置表单可以滚动到最后一个操作按钮，并校验窗口内五组菜单的三语标题。
...
- 当前 macOS ARM64 `.app` 窗口内统一菜单实测：Harness 工作台顶部显示“文件 / 编辑 / 视图 / 窗口 / 帮助”，Runtime 内容自动下移且不重叠；点击“视图”通过受管同源保留路径弹出系统原生菜单，显示工作台、桌面管理与全屏命令。Runtime WebView 仍不具备 Tauri IPC 权限。
...
- 上述结果证明当前源码、生成配置、锁定 Runtime、前端、本机 Runtime 启停链路与 macOS ARM64 安装版可运行；窗口内菜单的 macOS 成品布局和原生弹出项已实测。Windows 设置页滚动与统一菜单标题由对应尺寸的浏览器回归覆盖，仍不能替代 Windows/Linux 原生应用真机视觉和菜单行为验收。
