...
同步官方 Runtime `dsh-v0.1.2-alpha.1`（`cd5ef8148158c3a752a658978873241fdf8e2bbc`）并完成分布式本地发布系统后：
...
- `corepack pnpm@11.7.0 verify`：通过；配置与发布协议测试 31 项、Vue 测试 6 项、Rust 测试 33 项、Clippy `-D warnings`、3 个 locale / 99 个 key、凭据代理回归和 Runtime manifest 校验全部通过。
...
- 它不等于 Apple 公证、Windows 发布者签名、真实 macOS x64 / Windows x64 / Linux x64 节点协作或所有外部 Provider 已通过验收。
