...
- `v1.0.3` 托管发行流水线 `33179181096` 全部通过：公共质量门禁、macOS ARM64、macOS x64、Windows x64、Linux x64 和 Release 汇总均成功。预发行 Release 包含 5 个安装制品与 `SHA256SUMS`；下载校验清单后与 GitHub 返回的 SHA-256 摘要逐项比较，5 项全部一致。该结果证明原生平台构建与制品门禁通过，不替代用户侧真机安装和业务验收。
