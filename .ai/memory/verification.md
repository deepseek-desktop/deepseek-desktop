...
- macOS “视图”菜单崩溃回归：`1.0.23` 安装版可稳定重现 `EXC_BAD_ACCESS` / `SIGSEGV`，崩溃栈由 `objc_loadWeakRetained` 进入 Tao `mouse_moved` / `mouse_motion`，证明不是菜单命令主动退出。改为 macOS 光标位置弹出、不将带坐标菜单绑定到 Tao 根 `NSView` 后，从 `1.0.24-test.1` DMG 复制并通过 LaunchServices 启动的真实应用完成 100 次“视图”菜单循环和 200 次五组菜单循环，Desktop 与 Runtime 全程存活，无新增 `.ips`。同一成品上的“视图 -> 全屏”、退出全屏、关闭确认取消和确认退出均实测通过。
- `DESKTOP_APP_VERSION=1.0.24-test.1 corepack pnpm@11.24.0 desktop:package`：通过完整 `verify`、E2E、Runtime smoke、Tauri 发布编译和 DMG 构建；DMG 经 `hdiutil verify` 通过，应用经 `codesign --verify --deep --strict` 通过，主二进制为原生 `arm64`，`CFBundleShortVersionString` 与 `CFBundleVersion` 均为 `1.0.24-test.1`。该测试版本仅用于本地验收，未创建 Tag 或 Release。
