...
2026-09-22，四平台公共打包检查改为进程内共享一次 Harness 同步与暂存。免费 Runner 方案、优化前矩阵耗时和缓存边界统一见 [发布手册](../skills/release-workflow.md#免费-runner-提速方案)。

- 本机 macOS arm64 独立执行 `verify` 通过：142 项配置/发行测试、33 项前端测试、45 项搜索测试、113 项 Rust 测试（2 项联网测试默认忽略）、三语检查、类型检查与 Clippy。新增行为回归覆盖一次准备供完整检查复用、独立入口重新准备、准备失败阻断消费、失败阶段计时。
- 独立 `test:e2e` 6 项通过；`harness:smoke` 的插件/搜索设置、WebKit 模型与推理等级选择、父进程退出清理通过；发行身份与资产相关回归 26 项通过。
- 本机 `desktop:package` 完整链路通过，日志逐项核对 `harness:sync`、官方 `build:official`、`harness:stage` 均仅执行一次，全部检查保留。内部 BUILD-INFO 记录约 5 分 20 秒（同步 113.5 秒、暂存 20.8 秒、Tauri 57.4 秒、DMG 13.1 秒）；本机使用已有下载和工具链缓存，此值不是 GitHub 冷启动耗时或跨平台提速比例。分阶段 Markdown 摘要实际生成，失败摘要由回归测试覆盖。
- 本轮 ARM64 DMG 为 `local` 通道、`dirty=true` 的验证包，未发布或覆盖已安装应用。SHA-256 为 `127b511a03db8d0f38331fd44226a68533a6c5ea96996a50c7e4d651181cf953`；交付扫描、SHA256SUMS 重算、`hdiutil verify` 和包内应用 `codesign --verify --deep --strict` 均通过，仍不代表 Developer ID 签名或公证。
- 本次尚未运行 GitHub 四平台矩阵；不得把下方旧发行成功或本机耗时写成新流程的四平台提速实测。
