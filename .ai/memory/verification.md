...
- 上述结果证明当前源码、生成配置、锁定 Runtime、前端、本机 Runtime 启停链路与 macOS ARM64 成品应用可运行。本轮没有写入 `/Applications`，而是直接启动构建目录内的 `.app`；Windows 设置页滚动由 Windows 尺寸的浏览器回归覆盖，仍不能替代 Windows 原生应用真机验收。
