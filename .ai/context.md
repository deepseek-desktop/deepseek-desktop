...
- 当前成功发行是 `v1.1.20` 社区预发布；质量门禁、四平台原生构建、Windows x64 安装交互、汇总发布和六个公开文件下载验收均成功。公开包仍未使用可信发行签名，保持 prerelease 且不占据 Latest。`v1.1.14` 至 `v1.1.17` 和 `v1.1.21` 均为未发布的不可变失败 Tag。`v1.1.22` 候选继续使用官方 Harness `0.1.6-alpha.2`，补齐 Linux 官方 WASM 回退校验后须重新执行本机门禁与远端矩阵。证据范围见 [当前发布验收](memory/verification.md#当前发布验收)，防复发规则见 [发布手册](skills/release-workflow.md#最短反馈路径)。
