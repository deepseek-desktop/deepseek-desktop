...
- Harness 更新只写入应用数据目录；仓库模式复用内置 Node/pnpm/npm 和 Node-API 头拉取、按官方原生包流程构建并 smoke 候选，macOS/Linux 根据当前平台声明预检系统编译器；可选签名制品模式继续执行签名、兼容和受限解压校验，两者共用原子切换与自动回滚。安装包内置 Harness 始终作为最终恢复基线。设置 schema 只保存可选仓库覆盖值，切换仓库或频道会使旧候选失效并清空界面中的候选/待安装版本和进度，当前运行版本不变；诊断导出不包含仓库地址。
...
- 插件配置与只读插件清单使用 RC 的官方设置界面，搜索卡片注册到 `settings.plugin.item`，不混入 alpha 版本的插件管理器接口。DSH Market 仍是用户插件，不进入内置闭包；首次启动或 Harness commit 变化时，Desktop 用自己的 `DSH_HOME`、当前 Node/CLI 和随包 pnpm 执行 `dsh plugin --profile desktop-web add dshmarket@latest`。成功后按 commit 记账，失败提示并在下次普通启动重试；崩溃恢复及当次内置恢复不联网。CLI 管理依赖和 Bundle 声明，保留用户禁用状态，不改写市场代码。见 ADR-028。
...
- 新发行体系使用四段公开版本：锁定 Harness `0.1.5-rc.2` 对应 Desktop `0.1.5.<修订号>`，当前默认版本为 `0.1.5.1`；旧版本体系的 Release 与 Tag 已从 GitHub 清理，更新器只接受四段格式。
