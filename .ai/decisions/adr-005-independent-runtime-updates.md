...
待安装 Runtime 在下次启动前必须真实启动本地服务并通过 readiness、HTTP 探活和工作区注册。切换后无法启动或连续恢复达到上限时先回滚上一版；上一版无效时移除外部指针并使用安装包内置 Runtime。只保留当前、上一版和待安装版本，失败与启动时清理 staging 和孤立目录。用户可以固定当前版本或手动恢复内置版，离线恢复不依赖更新服务。
