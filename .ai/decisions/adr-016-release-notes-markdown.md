...
GitHub API 返回 Markdown，限流时备用 Atom feed 返回 HTML；格式由 Desktop 获取路径标记，不能由远端 JSON 覆盖。Atom 内容在浏览器原生惰性 template 中解析，只重建有限的排版元素、转义后的文字和已校验的链接；不挂载原 DOM，不复制事件、样式、命名空间或其他属性，不加载图片/iframe。两种来源共用布局与外链入口。保留网络响应 1 MiB 上限，不再按 1200 字符截断正文，以免破坏列表、表格或链接。

## 验证

覆盖 Markdown/Atom 结构、原始 HTML/脚本/危险 URL、命名空间与属性注入、远程图片、外链委派与失败提示。浏览器 E2E 使用模拟更新 IPC，验证两类摘要、无远程资源请求、弹窗真实布局、键盘链接、长内容及最小窗口尺寸；模拟 IPC 不作为原生操作系统浏览器启动验收。
