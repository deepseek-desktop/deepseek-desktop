...
已采用，取代 ADR-007 中依赖 Harness 搜索核心和官方设置 UI 补丁的实现。ADR-021 曾短暂取代其中「官方插件不强制停用」一条，该取代已由 [ADR-022](adr-022-search-mode-selection.md) 撤销；本 ADR 全部决策继续有效，仅模式命名由 ADR-022 更新。

## 决策

- 官方 `web-search-deepseek` 保持上游源码、配置与设置卡片不被改写，且与本扩展同时启用；用户 profile 的主动启停仍然保留。
- follow-model 自有 host/client、设置 namespace、三语资源与 `settings.plugin.item` 插槽。独立选择协调器提供“跟随当前模型 / 网页搜索 / 关闭联网搜索”三种模式（ADR-022 将第二项由自由填写的 Provider ID 改为直指官方插件），并将单一用户选择映射到 Harness 的 `web.searchProvider`；公开 `tools.guard` 在最终执行边界拒绝关闭或尚未生效的搜索，网页抓取不受影响，不维护第二套路由状态。删除此前对 web、tool-web、llm-pi-ai、llm-deepseek 和官方插件设置 UI 的搜索专用补丁；其他 Desktop 修复不属于本次删除范围。
...
启用状态与搜索选择分离，使官方插件可共存；单一用户选择通过 Harness 原生路由生效，避免 Harness 更新替换 Desktop 私有 UI 补丁后设置消失。每次跟随模型搜索绑定当前 Agent 的路由，不使用全局最近模型或全局凭据缓存。选择网页搜索时使用官方插件自己的配置和凭据，不借用当前会话模型凭据。
