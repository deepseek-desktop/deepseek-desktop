...
已采用，取代 ADR-007 中依赖 Harness 搜索核心和官方设置 UI 补丁的实现。其中「官方插件不强制停用」一条已由 ADR-021 取代，其余决策继续有效。

## 决策

- 官方 `web-search-deepseek` 保持上游源码、配置与设置卡片不被改写。**启用状态部分已由 ADR-021 取代**：桌面 profile 默认停用该插件，并由 `web-search-follow-model` 的 `officialSearchPlugin` 开关控制；用户 profile 的主动启停仍然保留。
