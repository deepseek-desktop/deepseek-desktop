# ADR-012：Harness 仓库请求继承 macOS 系统代理
...
- 仅为 Harness 的 HTTP(S) Git 检查和浅克隆补充进程级代理；SSH、本地仓库及 Harness 对话连接不变。
...
- 仓库检查有独立的 30 秒预算；超时状态有三语网络/代理提示，终止 Git 及其辅助进程，保持当前 Harness 不变。
