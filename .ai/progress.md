...
- 独立搜索扩展已移除搜索核心/官方设置补丁和强制停用官方插件的覆盖；自有设置通过公共插槽显示、保存、重载恢复。官方与 follow-model 同时启用时由 `web.searchProvider` 唯一选择一个 Provider，不会双重搜索；用户在「跟随当前模型 / 网页搜索 / 禁用联网搜索」之间互斥单选或恢复默认，用户主动停用官方插件仍被保留。重载失败会恢复之前的持久化值和实际路由。内置 Harness 已升级到官方 `0.1.5-rc.2` / `c291e7961a51`；桌面扩展按公开 peer 契约叠加，不再强行保留旧市场、旧设置表单或旧 RPC 注入机制。生产路径清理不改变二进制偏移，修改后的 Mach-O 会重新签名。
