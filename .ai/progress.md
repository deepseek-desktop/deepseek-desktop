...
- Harness WebKit 消息回放错误已定位为内置构造函数文本排版差异，在唯一装配链路修复，官方搜索插件未改。新 DMG 完整构建、实际安装历史恢复和同源链接复验通过；候选 Git HEAD 附加 ref 解析已补真实仓库回归。用户授权 Alibaba MaaS 后，Max / Flash 两个原生 GUI 会话搜索重叠 8067 毫秒，各返回 8 条结构化来源并正常完成。剪贴板最终复验未闭环，Windows x64 官方矩阵未执行，尚不发布 1.1.0。
