...
- 独立搜索扩展已移除五项搜索核心/官方设置补丁和强制停用官方插件的覆盖；自有设置通过公共插槽显示、保存、重载恢复。官方与 follow-model 同时启用时由 `web.searchProvider` 唯一选择后者，不会双重搜索；用户主动停用官方插件仍被保留。内置基线完整打包、社区 `76fda729799f` 与官方 `d347e703908d` 的实际装配、真实供应商调用及 macOS 安装包整链已通过。未固定的最新官方 Harness 仍被既有模型设置补丁兼容性阻断，Windows x64 真机环境不可用，因此本轮只完成本地提交，不发布。
