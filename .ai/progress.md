...
- 当前已成功交付的版本为 `v1.1.13` 社区预发布，commit `c23a2304`；Run `34316055136` 的质量门禁、四平台原生构建和发布全部成功，六个公开资产齐全。`v1.1.14` 的 Run `34693095400` 在 shell-quality 发现官方原生平台包未完整构建，因此未进入原生矩阵、未创建 Release，Tag 保持不可变。修复后的 `v1.1.15` 候选已通过本机全链、真实仓库候选构建和最终源码 Linux x64 容器预检，仍须从干净提交完成本机安装包与 Tag 原生矩阵。最新证据与未验证边界见 [当前发布验收](memory/verification.md#当前发布验收)，后续必须重读远端，不将该版本硬编码为下一次发行基线。
...
- macOS contentView 查询的所有权失衡已有修改前后因果证据与实际 DMG 回归；WebKit 历史恢复、同源链接、剪贴板、候选切换及 Alibaba MaaS Max / Flash 并发搜索的既有证据见 [审计修复验收](memory/audit-remediation.md)。下列标注旧版本的内容是历史能力验证，不代表最新安装状态或全部平台重新验收。

- 独立搜索扩展已移除搜索核心/官方设置补丁和强制停用官方插件的覆盖；自有设置通过公共插槽显示、保存、重载恢复。官方与 follow-model 同时启用时由 `web.searchProvider` 唯一选择一个 Provider，不会双重搜索；用户可以切换已注册的独立搜索 Provider、关闭搜索或恢复默认，用户主动停用官方插件仍被保留。重载失败会恢复之前的持久化值和实际路由。内置 Harness 已升级到官方 `0.1.5-rc.2` / `c291e7961a51`；桌面扩展按公开 peer 契约叠加，不再强行保留旧市场、旧设置表单或旧 RPC 注入机制。生产路径清理不改变二进制偏移，修改后的 Mach-O 会重新签名。
...
- Harness 仓库更新复用打包链路的生产 deploy，按依赖关系补齐桌面扩展且不覆盖候选核心；桌面组件适配新版设置服务，工作台跨启动代次清理受管认证 Cookie。一次性启动门闩保证候选激活完成后才启动服务。本机已使用应用内置 Node/pnpm/npm 与 Node-API 头，从官方 c291 源码完整构建 `0.1.5-rc.2` 仓库候选，验证核心包、桌面扩展、兼容契约和 ARM64 原生模块；其他平台的仓库更新交互仍待真机实测。
...
- Harness 已同步到官方仓库 `0.1.5-rc.2`（`c291e7961a515f6d7af9304e7fd1d257929aef26`）。生产闭包从官方 CLI、公开依赖与 `prebuilds.json` 解析；平台原生包执行完整上游 `build:native` 并由固定 npm 打包，桌面兼容修正只允许精确版本加已校验补丁或上游已原生满足契约。
...
- 工具链锁统一要求四个平台使用 Node `24.20.0` / ABI `137`、pnpm `11.24.0` 和随 Node 归档固定的 npm `11.19.0`；各官方 Runner 在打包前验证实际版本，缓存与内部 BUILD-INFO 同时绑定目标 triple 和工具链身份。
...
- Harness 更新页只显示一个仓库地址：默认使用构建时的官方 Harness 仓库，用户可替换为自己的兼容 fork。仓库模式读取默认分支 HEAD，在应用数据目录复用内置 Node/pnpm/npm 与 Node-API 头，并按官方原生包声明预检系统编译器、构建和 smoke 候选；设置只使用 Harness 字段，不提供历史迁移。诊断导出剔除仓库地址。可选预构建签名制品通道继续保留为维护者能力。
