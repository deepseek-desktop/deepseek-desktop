...
- 单机四环境源码交付改用 tag/commit 锁定并经 `git bundle verify` 校验的本地 bundle；统一 Runtime 部署显式携带四目标 Koffi、Sharp/libvips 等可选原生包，Rust 编译路径重映射规则进入 Cargo 缓存身份，避免节点 SSH 依赖、跨架构缺包和本机路径泄漏。
