...
- Harness WebKit 消息回放错误已定位为内置构造函数文本排版差异，在唯一装配链路修复，官方搜索插件未改。新 DMG 完整构建及实际安装历史恢复通过；候选 Git HEAD 附加 ref 解析已补真实仓库回归。最终真实并发搜索受测试账号 HTTP 402 余额不足阻塞，Windows x64 官方矩阵未执行，尚不发布 1.1.0。
