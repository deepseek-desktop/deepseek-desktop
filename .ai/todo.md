...
- Windows x64 与 Linux x64 的窗口内菜单验收：顶部左侧唯一显示“文件 / 编辑 / 视图 / 窗口 / 帮助”，原生弹出项、应用命令、编辑命令、最大化/最小化/全屏和高 DPI 布局均可用，且系统窗口不再重复显示完整菜单。自动化覆盖菜单构建、三语文案、逻辑/物理像素边界和设置事件，不以浏览器模拟代替真机结论。
