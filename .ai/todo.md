...
- Harness 仓库更新依赖闭包：本机 v1.0.30 已完成默认仓库最新候选的下载和构建，但候选 smoke 提前退出。隔离复现确认桌面扩展的 `yaml` 与上游 peer 依赖未随包复制完整；需修复候选闭包装配并验证更新、激活和回滚，不能把发现新 commit 当作更新成功。当前内置内核与用户数据保持不变。
