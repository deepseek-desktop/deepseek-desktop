...
- Harness util-values 的 WebKit 历史回放问题已修复，见 ADR-020；完整打包、双引擎回归及新 DMG 历史恢复通过。同源导航最终包复验及 Alibaba MaaS 两个模型真实并发搜索已通过；最终包候选 Harness 切换与剩余门禁仍需复核。
...
- 剪贴板复验及本地门禁全通过后才能创建新 annotated Tag；Windows x64 原生安装、启动交互、退出和卸载必须在官方矩阵通过后才发布。当前不创建 Tag / Release，不移动旧 Tag。
