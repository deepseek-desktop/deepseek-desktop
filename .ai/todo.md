...
- `v1.1.0` / `8c0a503` 的 Run `33969114177` 在容器身份校验前因 Git dubious ownership 失败，原生矩阵和发布均未运行。已将安全目录登记限定到该临时 CI Job 的实际工作区，回归覆盖修复前拒绝、修复后完整 Tag 校验和其他仓库仍被拒绝；不移动失败 Tag。下一候选为未占用的 `1.1.1`，仍须全部矩阵通过才发布。
