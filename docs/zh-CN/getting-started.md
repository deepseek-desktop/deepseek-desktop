...
联网搜索默认跟随当前会话使用的模型。Provider 声明标准搜索协议后，Runtime 会复用它已经保存的 API 地址、模型和凭据引用，不要求用户再次填写搜索密钥；切换模型后，下一次搜索同步切换。未声明或明确不支持搜索的 Provider 会给出可操作提示，正常对话仍可继续，也不会为了猜测协议向接口发送额外请求。Provider 开发者和高级用户可查看[跟随当前模型的联网搜索](runtime-web-search.md)。
