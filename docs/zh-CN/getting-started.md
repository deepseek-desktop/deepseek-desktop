...
社区版采用官方 Harness 的插件配置和只读插件列表，可查看全局与 Agent 预设的插件组合、启停状态及加载失败原因。桌面版不再强制装配 DSH Market；插件列表不提供市场安装、更新或卸载功能。Harness 每次启动会合并桌面内置 Bundle 和用户插件配置，已有的自定义插件声明仍会保留；已撤出内置包且能通过 Desktop 所有权标记和内容摘要确认未被修改的旧受管 Bundle，只撤下启用声明，保留文件。用户显式依赖或已修改的包不会被自动撤下。固定版本 pnpm 随应用提供，供官方插件命令和候选构建使用。
...
Harness 更新可在“设置 → 更新 → Harness 独立更新”中选择“自动下载并在下次启动安装”“发现后提醒”或“仅手动检查”，也可以固定当前版本。设置页默认显示官方 Harness 仓库；用户可替换为自己的兼容 fork，不需要配置额外清单、公钥或发布者。Desktop 会在应用数据目录使用内置 Node/pnpm 准备候选并执行真实启动 smoke，通过后才在下次启动切换；失败会保留当前版本并自动回滚，用户也可随时恢复安装包内置 Harness。更详细的行为与离线恢复说明见 [Harness 独立更新指南](harness-updates.md)。
