...
点击“检查 Harness”后，Desktop 用 Git 读取所选仓库默认分支的 `HEAD`。发现 commit 变化后，用户确认准备，Desktop 会在应用数据目录中浅克隆仓库，复用安装包内置的 Node `24.20.0` 和 pnpm 安装锁定依赖、执行仓库的 `build`，再补齐桌面凭据代理和市场组件。用户不需要单独安装 Node 或 pnpm，但系统需要能够执行 Git；私有仓库的访问权限由用户自己的 Git 环境负责。
