...
test("onboarding, language switching, and status views fully load", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "欢迎使用 DeepSeek Desktop" })).toBeVisible();
  await page.getByLabel("切换语言").selectOption("en-US");
  await expect(page.getByRole("heading", { name: "Welcome to DeepSeek Desktop" })).toBeVisible();
  ...
  await expect(page.getByText("0.1.0-community.6")).toBeVisible();
