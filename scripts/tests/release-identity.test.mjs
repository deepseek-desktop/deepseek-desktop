...
test("release identity rejects lightweight, moved, mismatched and replaced annotated tags", async () => {
  ...
  try {
    ...
    git("tag", "v0.1.6.1");
    const input = { root, tag: "v0.1.6.1", version: "0.1.6.1", remote: root, commit: git("rev-parse", "HEAD") };
    assert.throws(() => releaseIdentity(input), /annotated/u);
    git("tag", "-d", "v0.1.6.1");
    git("-c", "user.name=Test", "-c", "user.email=test@example.invalid", "tag", "-a", "v0.1.6.1", "-m", "release");
    const identity = releaseIdentity(input);
    assert.equal(identity.commit, input.commit);
    assert.throws(() => releaseIdentity({ ...input, version: "0.1.6.2" }), /version mismatch/u);
    assert.throws(() => releaseIdentity({ ...input, commit: "0".repeat(40) }), /commit mismatch/u);
    git("tag", "-d", "v0.1.6.1");
    git("-c", "user.name=Test", "-c", "user.email=test@example.invalid", "tag", "-a", "v0.1.6.1", "-m", "changed");
...
test("container checkout trust preserves annotated tag checks without trusting other repositories", { skip: process.platform === "win32" }, async () => {
  ...
  try {
    ...
    git(source, "-c", "user.name=Test", "-c", "user.email=test@example.invalid", "tag", "-a", "v0.1.6.1", "-m", "release");
    const object = git(source, "rev-parse", "refs/tags/v0.1.6.1");
    const commit = git(source, "rev-parse", "HEAD");
    git(checkout, "init");
    // Match checkout's explicit shallow tag fetch, including the annotated object.
    git(checkout, "fetch", "--no-tags", "--depth=1", pathToFileURL(source).href, "+refs/tags/v0.1.6.1:refs/tags/v0.1.6.1");
    git(checkout, "checkout", "--detach", "refs/tags/v0.1.6.1");
    ...
    assert.throws(() => execute("git", ["cat-file", "-t", "refs/tags/v0.1.6.1"]), /dubious ownership/u);
    ...
    const input = { root: checkout, tag: "v0.1.6.1", version: "0.1.6.1", commit, expectedObject: object };
