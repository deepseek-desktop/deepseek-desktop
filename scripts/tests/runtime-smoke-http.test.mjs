...
import { requestLoopback, waitForLoopback } from "../../runtime/scripts/loopback-http.mjs";
...
test("loopback readiness retries until the Runtime accepts HTTP requests", async t => {
...
test("loopback readiness stops when the Runtime has already exited", async () => {
  await assert.rejects(
    () => waitForLoopback("http://127.0.0.1:9/", {
      child: { exitCode: 1, signalCode: null },
      timeoutMs: 100
    }),
    /runtime exited before/u
  );
});
