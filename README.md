...
One host builds only its native target. Cross-platform GitHub Releases are created from a matching annotated version tag such as `v0.1.0-community.4`; the release workflow publishes only after the macOS arm64/x64, Windows x64, and Linux x64 jobs all succeed.
