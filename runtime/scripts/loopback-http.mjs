...
export async function waitForLoopback(value, {
  ...
  while (Date.now() < deadline) {
    if (child && (child.exitCode !== null || child.signalCode !== null)) {
      throw new Error(`runtime exited before its loopback endpoint accepted requests (code=${String(child.exitCode)}, signal=${String(child.signalCode)})`);
    }
    try {
  ...
  throw new Error(`runtime loopback endpoint did not become ready within ${timeoutMs}ms: ${lastError?.message || "unknown error"}`, {
    cause: lastError
  });
}
