# Third-Party Notices

The packaged runtime includes DeepSeek Harness and Node.js. Both are distributed under the MIT License.

- DeepSeek Harness `0.1.1-rc.2`: <https://github.com/deepseek-ai/deepseek-harness>
- Node.js `24.16.0`: <https://github.com/nodejs/node>; the official archive license is packaged as `licenses/node-LICENSE.txt`.

The build writes the complete production dependency license inventory and checksums into each staged runtime directory. DeepSeek Harness Desktop source code is licensed under Apache-2.0.
